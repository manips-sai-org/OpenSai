...
The redis keys are decomposed in namespaces. The default namespace prefix is `sai::interfaces::` if not specified in the config file. In all Opensai examples, the prefix will be `opensai::`. The redis keys are then constructed using the names of the robots or objects, the controller names, task names and finally the specific key. For example, the key to change the gains of the task named `joint_task` in the controller named `cartesian_controller` for the robot `Panda` will be `opensai::controller::Panda::cartesian_controller::joint_task::kp`. You can open a redis client in terminal to look at the redis keys and get their value to print using the `get` command in order to examine them. You can also set a key with the `set` command. After running the main example, open a new terminal and:
```
redis-cli
127.0.0.1:6379> keys *
